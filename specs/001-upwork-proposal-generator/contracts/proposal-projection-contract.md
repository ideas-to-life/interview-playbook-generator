...
## Outputs
- `out/<target-slug>/upwork-proposal.md` (Clean prose ready for submission)
...
2. **`CONDITIONAL` State (`proposal_generation: allowed_with_conditions`)**:
   - `upwork-proposal.md` may render a draft proposal provided unsupported claims are excluded.
...
3. **`APPLY` State (`proposal_generation: allowed`)**:
   - Renders submission-ready `upwork-proposal.md` (350-500 words target) as clean, natural professional prose (no `[evidence]` tags or `[^source-id]` footnotes in visible proposal text).
