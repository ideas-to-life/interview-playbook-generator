...
- Q: When migrating existing canonical OKF EvidenceCards to the V2.0 machine-readable metadata contract, should metadata values (organisation.id, project.id, environment, production_verified, implementation_role) be populated strictly from explicit source evidence for each individual EvidenceCard, representing unevidenced or ambiguous values as unknown/false rather than inferring them from broader employment records or project names? → A: Option A - Yes, strictly populate frontmatter metadata from explicit source evidence per card; any value not directly evidenced in source documents for that card MUST be set to unknown (or production_verified: false), with zero contextual inheritance or inference.
...
If canonical evidence does not explicitly establish the deployment environment in frontmatter, the environment status MUST remain `unknown`. The system MUST NOT infer `production` from free-text prose or filenames. When migrating existing canonical OKF EvidenceCards to V2.0 frontmatter metadata, fields MUST be populated strictly from explicit source evidence per individual card; unevidenced or ambiguous values MUST be set to `unknown` (or `production_verified: false`), with zero contextual inheritance or inference from parent employment records or project folders.
