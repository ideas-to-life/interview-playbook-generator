...
- **FR-034 (No Responsibility-Ladder Inference)**: Complements FR-033 by prohibiting inferential upgrades across contribution levels. The system MUST NOT infer `implemented` from `architected`, `deployed` from `implemented`, `production deployment` from `deployment`, `candidate implementation` from `organisation production`, or `personal implementation` from `team implementation`.
